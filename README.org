...
| TERM    | HAS                     | Meaning                                                                                 |
|---------+-------------------------+-----------------------------------------------------------------------------------------|
| Session | program input/output    | a run of a program with input/output                                                    |
|         |                         | appear in ~.org~ test files as ~#+BEGIN_SRC: / #+END_SRC:~                              |
|---------+-------------------------+-----------------------------------------------------------------------------------------|
| Segment | a single session and    | combination of some description, options, and a single session                          |
|         | options/description     | appear in ~.org~ files as some descriptive text, ~#+TESTY:~ directives, then a session  |
