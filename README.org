...
The basic structure of Org files is an outline.
...
- Tags or "directives" in org-mode start with ~#+~ and the most
  important tag in the format is the ~#+BEGIN_SRC~ and ~#+END_SRC~
  pair which denote a test *session*, which shows prompts with input
